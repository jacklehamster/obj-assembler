...
function actualType(path) {
  var actualType = strEndsWith(path, ".json") ? "json" : strEndsWith(path, ".png") || strEndsWith(path, ".jpg") || strEndsWith(path, ".jpeg") ? "image" : strEndsWith(path, ".mp3") ? "audio" : strEndsWith(path, ".csv") ? "csv" : strEndsWith(path, ".yaml") || strEndsWith(path, ".yml") ? "yaml" : "text";
  return actualType;
}
...
function loadAll$1$1(input, iterator, options) {
  if (iterator !== null && typeof iterator === 'object' && typeof options === 'undefined') {
    options = iterator;
    iterator = null;
  }
...
function load$1$1(input, options) {
  var documents = loadDocuments$1(input, options);
...
var loadAll_1$1 = loadAll$1$1;
var load_1$1    = load$1$1;
...
var DEPRECATED_BASE60_SYNTAX$1 = /^[-+]?[0-9_]+(?::[0-9_]+)+(?:\.[0-9_]*)?$/;
...
var QUOTING_TYPE_SINGLE$1 = 1,
    QUOTING_TYPE_DOUBLE$1 = 2;
...
function State$2(options) {
  ...
  this.noCompatMode  = options['noCompatMode'] || false;
  this.condenseFlow  = options['condenseFlow'] || false;
  this.quotingType   = options['quotingType'] === '"' ? QUOTING_TYPE_DOUBLE$1 : QUOTING_TYPE_SINGLE$1;
  this.forceQuotes   = options['forceQuotes'] || false;
  this.replacer      = typeof options['replacer'] === 'function' ? options['replacer'] : null;
...
function isPrintable$1(c) {
  return  (0x00020 <= c && c <= 0x00007E)
      || ((0x000A1 <= c && c <= 0x00D7FF) && c !== 0x2028 && c !== 0x2029)
      || ((0x0E000 <= c && c <= 0x00FFFD) && c !== CHAR_BOM$1)
      ||  (0x10000 <= c && c <= 0x10FFFF);
}
...
// Including s-white (for some reason, examples doesn't match specs in this aspect)
// ns-char ::= c-printable - b-line-feed - b-carriage-return - c-byte-order-mark
function isNsCharOrWhitespace$1(c) {
...
function isPlainSafe$1(c, prev, inblock) {
  var cIsNsCharOrWhitespace = isNsCharOrWhitespace$1(c);
  ...
  return (
    ...
    || (isNsCharOrWhitespace$1(prev) && !isWhitespace$1(prev) && c === CHAR_SHARP$1) // change to true on '[^ ]#'
...
function isPlainSafeFirst$1(c) {
  ...
  // where ns-char = nb-char - s-white.
  // No support of ( ( “?” | “:” | “-” ) /* Followed by an ns-plain-safe(c)) */ ) part
  return isPrintable$1(c) && c !== CHAR_BOM$1
...
// Simplified test for values allowed as the last character in plain style.
function isPlainSafeLast$1(c) {
  // just not whitespace or colon, it will be checked to be plain character later
  return !isWhitespace$1(c) && c !== CHAR_COLON$1;
}

// Same as 'string'.codePointAt(pos), but works in older browsers.
function codePointAt$1(string, pos) {
  var first = string.charCodeAt(pos), second;
  if (first >= 0xD800 && first <= 0xDBFF && pos + 1 < string.length) {
    second = string.charCodeAt(pos + 1);
    if (second >= 0xDC00 && second <= 0xDFFF) {
...
function chooseScalarStyle$1(string, singleLineOnly, indentPerLevel, lineWidth,
  ...
  var shouldTrackWidth = lineWidth !== -1;
  var previousLineBreak = -1; // count the first line correctly
  var plain = isPlainSafeFirst$1(codePointAt$1(string, 0))
          && isPlainSafeLast$1(codePointAt$1(string, string.length - 1));

  if (singleLineOnly || forceQuotes) {
    // Case: no block styles.
    // Check for disallowed characters to rule out plain and single.
    for (i = 0; i < string.length; char >= 0x10000 ? i += 2 : i++) {
      char = codePointAt$1(string, i);
  ...
  if (!hasLineBreak && !hasFoldableLine) {
    ...
    return quotingType === QUOTING_TYPE_DOUBLE$1 ? STYLE_DOUBLE$1 : STYLE_SINGLE$1;
  }
  // Edge case: block indentation indicator can only have one digit.
...
function writeScalar$1(state, string, level, iskey, inblock) {
  state.dump = (function () {
    if (string.length === 0) {
      return state.quotingType === QUOTING_TYPE_DOUBLE$1 ? '""' : "''";
    }
    if (!state.noCompatMode) {
      if (DEPRECATED_BOOLEANS_SYNTAX$1.indexOf(string) !== -1 || DEPRECATED_BASE60_SYNTAX$1.test(string)) {
        return state.quotingType === QUOTING_TYPE_DOUBLE$1 ? ('"' + string + '"') : ("'" + string + "'");
      }
    }
...
function dump$1$1(input, options) {
  options = options || {};
...
var dump_1$1 = dump$1$1;

var dumper$1 = {
	dump: dump_1$1
};

function renamed$1(from, to) {
  return function () {
    throw new Error('Function yaml.' + from + ' is removed in js-yaml 4. ' +
      'Use yaml.' + to + ' instead, which is now safe by default.');
  };
...
// Removed functions from JS-YAML 3.0.x
var safeLoad$1            = renamed$1('safeLoad', 'load');
var safeLoadAll$1         = renamed$1('safeLoadAll', 'loadAll');
var safeDump$1            = renamed$1('safeDump', 'dump');
...
var Loader = /*#__PURE__*/function () {
  ...
  _proto.get = function get(path, type) {
    try {
      var _temp3 = function _temp3() {
        ...
        cacheResult.pendingLoads.forEach(function (pendingLoad) {
          return pendingLoad(cacheResult);
        });
        return cacheResult;
      };
      var _this = this;
      var fixedPath = url.resolve(location.href, path);
      if (_this.cache[fixedPath]) {
        if (_this.cache[fixedPath].loaded) {
          return Promise.resolve(_this.cache[fixedPath]);
        } else {
          return Promise.resolve(new Promise(function (resolve) {
            return _this.cache[fixedPath].pendingLoads.add(resolve);
          }));
        }
      }
      var theType = type != null ? type : actualType(fixedPath);
      var cacheResult = _this.cache[fixedPath] = {
        pendingLoads: new Set(),
        loaded: false
      };
      var _temp2 = _switch(theType, [[function () {
        return "image";
      }], [function () {
        return "audio";
      }, function () {
        return Promise.resolve(_this.load(fixedPath, function (response) {
        ...
        })).then(function (_this$load) {
          cacheResult.object = _this$load;
        });
      }], [function () {
        return "yaml";
      }, function () {
        return Promise.resolve(_this.load(fixedPath, function (response) {
          var _response$yaml, _response$yaml2;
          return (_response$yaml = (_response$yaml2 = response.yaml) === null || _response$yaml2 === void 0 ? void 0 : _response$yaml2.call(response)) != null ? _response$yaml : response.text().then(function (text) {
            return jsYaml$1.load(text);
          });
        })).then(function (_this$load2) {
          cacheResult.object = _this$load2;
        });
      }], [function () {
        return "text";
      }], [void 0, function () {
        ...
        })).then(function (_this$load3) {
          cacheResult.text = _this$load3;
        });
      }]]);
  ...
  _proto.load = function load(path, transform) {
    try {
      var _this2 = this;
  ...
  _proto.clear = function clear() {
    var _this3 = this;
    Object.values(this.cache).forEach(function (cacheResult) {
      if (cacheResult.src) {
        URL.revokeObjectURL(cacheResult.src);
      }
    });
    Object.keys(this.cache).forEach(function (key) {
      return delete _this3.cache[key];
    });
  };
  return Loader;
}();
...
var AssetTransformer = /*#__PURE__*/function (_Transformer) {
  _inheritsLoose(AssetTransformer, _Transformer);
  function AssetTransformer(loader, factory) {
    var _this;
    _this = _Transformer.call(this) || this;
    _this.loader = loader;
    _this.assetFactory = factory;
    return _this;
  }
  ...
  _proto.process = function process(data, dir, property, params) {
    try {
      var _this2 = this;
      return Promise.resolve(_this2.assetFactory(data, _this2.loader, dir, property, params));
    } catch (e) {
      return Promise.reject(e);
    }
  };
...
var ObjTransformer = /*#__PURE__*/function (_AssetTransformer) {
  _inheritsLoose(ObjTransformer, _AssetTransformer);
  function ObjTransformer(loader, assembler) {
    var _this;
  ...
  var _proto = ObjTransformer.prototype;
  _proto.paramsReplacement = function paramsReplacement(obj, params) {
    if (!params) {
      return obj;
    }
    ...
    if (Array.isArray(obj)) {
      ...
      for (var _i in obj) {
        if (obj.hasOwnProperty(_i)) {
          obj[_i] = this.paramsReplacement(obj[_i], params);
        }
      }
    }
    return obj;
...
var REFERENCE_SEARCH_LIMIT = 3000;
var ReferenceTransformer = /*#__PURE__*/function (_Transformer) {
  _inheritsLoose(ReferenceTransformer, _Transformer);
  function ReferenceTransformer() {
    return _Transformer.apply(this, arguments) || this;
  }
  var _proto = ReferenceTransformer.prototype;
  _proto.process = function process(data, _, property, params) {
    try {
      var path = data.reference.startsWith("#") ? data.reference : "#" + url.resolve(property.substring(1), data.reference);
      return Promise.resolve(new Promise(function (resolve) {
        var start = Date.now();
        function findReference() {
          if (params.objects[path] !== undefined) {
            ...
            var error = "Unable to find reference for: " + path;
            console.warn(error);
...
var Assembler = /*#__PURE__*/function () {
  ...
  _proto.register = function register(type, transformer) {
    var _this = this;
    var types = Array.isArray(type) ? type : [type];
    types.forEach(function (type) {
      return _this.transformers.set(type, transformer);
    });
  };
  _proto.clear = function clear() {
    this.initialize();
